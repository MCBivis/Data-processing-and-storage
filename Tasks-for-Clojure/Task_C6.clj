...
(defn dijkstra [route-map from to]
  (letfn [(min-unvisited [dist visited]
            (let [candidates (remove visited (keys dist))]
              (when (seq candidates)
                (apply min-key dist candidates))))]
    (loop [dist {from 0}
           prev {}
           visited #{}]
      (if-let [u (min-unvisited dist visited)]
        (if (= u to)
          (letfn [(reconstruct [cur acc]
                    (if (= cur from)
                      (cons from acc)
                      (reconstruct (get prev cur) (cons cur acc))))]
            {:path (vec (reconstruct to '())) :price (dist to)})
          (let [neighbors (or (get-in route-map [:forward u]) {})
                [dist' prev'] (reduce
                               (fn [[d p] [v {:keys [price]}]]
                                 (let [alt (+ (get d u 0) price)
                                       cur-d (get d v Double/POSITIVE_INFINITY)]
                                   (if (< alt cur-d)
                                     [(assoc d v alt) (assoc p v u)]
                                     [d p])))
                               [dist prev]
                               neighbors)]
            (recur dist' prev' (conj visited u))))))))

(def transact-cnt (ref 0))

(defn filter-available [route-map]
  (let [fwd (:forward route-map)
        fwd2 (into {}
                   (for [[u neighb] fwd]
                     [u (into {}
                              (for [[v desc] neighb
                                    :let [tickets-ref (:tickets desc)]
                                    :when (pos? @tickets-ref)]
                                [v desc]))]))]
    (assoc route-map :forward fwd2)))
...
(defn book-tickets
  ...
  (if (= from to)
    {:path [] :price 0}
    (loop []
      ;; берём «срез» маршрутов, где ещё есть билеты
      (let [snapshot (filter-available route-map)
            best (dijkstra snapshot from to)]
        (if (nil? best)
          ;; если даже дёшевого пути не осталось — завершаем
          {:error :no-path}
          ;; иначе пробуем транзакционно снять билеты
          (let [{:keys [path price]} best]
            (dosync
             (alter transact-cnt inc)
             (let [edges (map vector path (rest path))
                   routes (map (fn [[a b]] (get-in route-map [:forward a b])) edges)]
               (if (every? #(pos? @(% :tickets)) routes)
                 (do (doseq [r routes] (alter (r :tickets) dec))
                     {:path path :price price})
                 ;; гонка: билеты успели кончиться, пробуем снова
                 (recur))))))))))
...
(defn run []
  ...
  (let [f1 (booking-future spec1 "City1" "City3" 10 1),
