...
(defn notify-msg
  ...
  ;; если фабрика не нуждается в этом ware — ничего не делаем
  (let [needed ((:bill state) ware)]
    (if (nil? needed)
      state
      (let [buffer       (:buffer state)
            have-now     (get buffer ware 0)
            need-more    (max 0 (- needed have-now))
            ;; попытка атомарно взять нужное количество из storage-atom
            taken
            (if (> need-more 0)
              (try
                (let [t (atom 0)]
                  (swap! storage-atom
                         (fn [old]
                           (let [take (min need-more old)]
                             (reset! t take)
                             (- old take))))
                  @t)
                (catch Exception _
                  ;; если swap! упал из-за validator или другой ошибки — считаем, что ничего не взяли
                  0))
              0)
            ;; обновлённый буфер (резервируем взятое)
            new-buffer (assoc buffer ware (+ have-now taken))
            state-with-buffer (assoc state :buffer new-buffer)]
        ;; проверяем, достаточно ли ресурсов для старта цикла
        (if (every? (fn [[w req]] (>= (get new-buffer w 0) req))
                    (:bill state))
          ;; ресурсы есть — запускаем цикл (имитация работы внутри агента)
          (do
            (Thread/sleep (:duration state))
            ;; после завершения цикла — списываем ресурсы из буфера
            (let [after-buffer
                  (reduce-kv (fn [buf w req] (update buf w #(- % req)))
                             new-buffer
                             (:bill state))
                  new-state (assoc state-with-buffer :buffer after-buffer)
                  target (:target-storage state)]
              ;; уведомляем целевой склад о произведённом количестве
              (send (target :worker) supply-msg (:amount state))
              ;; возвращаем обновлённое состояние фабрики
              new-state))
          ;; ресурсов пока недостаточно — возвращаем состояние с обновлённым буфером
          state-with-buffer)))))
